...
const (
	...
	googleAccessTokenHash = "piwt8oCH-K2D9pXlaS1Y-w"                                                    //nolint:gosec
	googleAccessToken     = "ya29.CjHSA1l5WUn8xZ6HanHFzzdHdbXm-14rxnC7JHch9eFIsZkQEGoWzaYG4o7k5f6BnPLj" //nolint:gosec
	googleSigningAlg      = RS256
	// following values computed by own algo for regression testing.
	computed384TokenHash = "_ILKVQjbEzFKNJjUKC2kz9eReYi0A9Of"            //nolint:gosec
	computed512TokenHash = "Spa_APgwBrarSeQbxI-rbragXho6dqFpH5x9PqaPfUI" //nolint:gosec
)
...
func TestAccessTokenVerification(t *testing.T) {
	...
	assertNil := func(err error) error {
		if err != nil {
			return fmt.Errorf("want nil error, got %w", err)
		}
		return nil
	}
	assertMsg := func(msg string) func(err error) error {
		return func(err error) error {
			...
			if err.Error() != msg {
				return fmt.Errorf("bad error message, %w, (want %q)", err, msg)
			}
			return nil
		}
	}
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			hf := func(w http.ResponseWriter, r *http.Request) {
				...
				io.WriteString(w, strings.ReplaceAll(test.data, "ISSUER", issuer)) //nolint:errcheck
			}
